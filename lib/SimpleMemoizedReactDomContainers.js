...
var SimpleMemoizedReactDomContainers = function () {
    ...
    _createClass(SimpleMemoizedReactDomContainers, [{
        key: 'getContainer',
        value: function getContainer(row, col, val) {
            return this.getMemoizedContainerRef(row, col, val) || this.createNewCellContainer(row, col, val);
        }
        ...
        value: function getMemoizedContainerRef(row, col, val) {
            var container = (0, _get2.default)(this.renderedContainers, [row, col, val, 'container'], null);
        ...
        value: function createNewCellContainer(row, col, val) {
        ...
        value: function memoizeContainer(container, row, col, val) {
            var renderedContainers = this.renderedContainers,
                renderedContainersIds = this.renderedContainersIds;

            (0, _setWith2.default)(renderedContainers, [row, col, val, 'container'], container, Object);

            if (renderedContainersIds.length >= this.MAX_CACHE) {
                this.removeFirstContainer();
            }

            renderedContainersIds.push({ row: row, col: col, val: val });
        }
    }]);
